...
describe("PostingView Comonent", () => {
  beforeEach(() => {
    ...
    apiCalls.patchEventPosting.mockResolvedValueOnce("event-2", {
      jobId: "posting-4",
    });
    apiCalls.postJobPosting.mockResolvedValueOnce(
      "event-2",
      _mockData.postJobBody
    );
  });
  ...
  it("should call getSinglePosting and getUser", async () => {
    ...
    render(
      <Router history={history}>
        <PostingView match={_mockData.eventId} getUserInfo={jest.fn()} />
      </Router>
    );
  ...
  it("should render correctly", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByAltText("return-home-button")).toBeInTheDocument()
    );
    await waitFor(() =>
    ...
    await waitFor(() =>
      expect(
        screen.getByText("Deliver food for a memorial hospital")
      ).toBeInTheDocument()
    );
    await waitFor(() =>
    ...
    await waitFor(() =>
      expect(screen.getByText("Open Spots: 2")).toBeInTheDocument()
    );
    await waitFor(() => expect(screen.getByText("cook")).toBeInTheDocument());
    await waitFor(() =>
      expect(screen.getByText("Open Spots: 3")).toBeInTheDocument()
    );
    await waitFor(() =>
      expect(screen.getByText("Sign me up!")).toBeInTheDocument()
    );
    await waitFor(() =>
      expect(screen.getByText("Feb 05 2021")).toBeInTheDocument()
    );
    await waitFor(() =>
    ...
    await waitFor(() =>
      expect(
        screen.getByText("123 Goose Blv., Denver, CO, 80208")
      ).toBeInTheDocument()
    );
    await waitFor(() =>
  ...
  it("should be able to sign up for one job per event and return to postings page", async () => {
    ...
    await waitFor(() =>
      screen.getByRole("button", { name: "Sign me up!" }).click()
    );

    await waitFor(() =>
      expect(screen.getByText("Open Spots: 1")).toBeInTheDocument()
    );
    await waitFor(() =>
      expect(screen.getByText("Sign me up!")).toHaveAttribute("disabled")
    );

    await waitFor(() =>
      expect(
        screen.queryByRole("button", { name: "Sign me up!" })
      ).not.toBeInTheDocument()
    );
    await waitFor(() =>
      expect(
        screen.getByText("You have already signed up for this event!")
      ).toBeInTheDocument()
    );

    await waitFor(() =>
      userEvent.click(screen.getByAltText("return-home-button"))
    );
